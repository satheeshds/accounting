...
var migrations = []string{
	...
	// Drop any leftover _bak from a prior crashed run before we start renaming (makes this crash-safe).
	...
	`DROP TABLE IF EXISTS transaction_documents_bak`,
	// Re-create indexes lost when transaction_documents was replaced by the rename above.
	`CREATE INDEX IF NOT EXISTS idx_transaction_documents_txn ON transaction_documents(transaction_id)`,
	`CREATE INDEX IF NOT EXISTS idx_transaction_documents_doc ON transaction_documents(document_type, document_id)`,
